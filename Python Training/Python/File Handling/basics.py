...
text = f.read()  #read() method moves the cursor to end when read fully
print(text)
# File Current Postions - fileobject.tell()
print("Current Position", f.tell())
...
f = open("filesio/files_smalltext.txt", "r")
text = f.readlines()
print(type(text)) #type -> list , for read() it is str type
print(text)
f.close()
...
print(f.closed) # returns a boolean value and tell whether a file is closed or not
...
with open("filesio/test.jpg", "rb") as fb:  #rb is read binary
    imageb = fb.read()
    print(imageb)


## After Reading this go to this link and read -> https://colab.research.google.com/drive/1TP7ks1pnEzJwwzHtswkSYvMWwo2HeRxM?usp=sharing
## OR Search CampusX File Handling video